        ...
        stage('Slack Notification') {
            steps {
                script {
                   ...
                   def qg = sh(returnStdout: true, script: 'curl -s -u admin:abhi "http://3.12.197.101:9000/api/qualitygates/project_status?projectKey=go" | jq -r .projectStatus.status').trim()
